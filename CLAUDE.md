...
- `apps/api/` — **FastAPI** (Python). REST + WebSocket surface for the dashboard, agent orchestration, MT5/MCP plane, per-pair Docker container control.
...
- `mcp/` — **MetaTrader 5 MCP server** (Python). **Already exists** — pre-dates the rest. This is the codebase that runs *inside* every per-pair Docker container; each container exposes its own MCP endpoint (recorded in `pairs.mcp_url`/`mcp_port`). Don't duplicate or rewrite — extend.
- `packages/` — shared code (TS types, schemas).
- Root config: `pnpm-workspace.yaml`, `pyproject.toml`.
- DB: **PostgreSQL** (forced by `pairs` + `agents` DDL using `gen_random_uuid()`, `JSONB`, `TIMESTAMP`, `TEXT[]`).
...
- Per-trade risk, daily max drawdown, total max drawdown, and simultaneous-exposure caps are **pair-level config values**. Default per-trade risk is 1%. Code should read these from pair config rather than hardcoding.
...
The system runs **multiple independent trading pairs simultaneously**. A *pair* (`pairs` table — the old `projects` "trading unit", renamed in the accounts-pairs restructure) is the unit that owns a symbol, timeframe, strategy, allocated capital, and risk parameters. Pairs are grouped under an **Account** (broker account + credentials), and accounts under an **Exchange** (the trading venue): the live hierarchy is **Exchange → Account (Cuenta) → Pair (Par) → Agents**. The Orchestrator manages all pairs concurrently. When adding features, prefer per-pair state and config; avoid global singletons that would conflate pairs.
...
- **`q_tables`** — per-pair versioned Q-value snapshots. `JSONB table` mapping `state_key → action → q_value`, with `version` monotonically increasing per `pair_id` and `UNIQUE (pair_id, version)`. Hyperparameters (`alpha_normal`, `alpha_special`, `gamma`) live on the row so a snapshot is fully reproducible from the row alone.
...
**Recovery loader**: `learning.recovery.RecoveryLoader` runs on FastAPI lifespan AFTER `recover_stale_runs` and BEFORE optional auto-wake. Idempotent, per-pair, user_id-aware — rebuilds the in-process `LearningCache` from disk so a fresh container boot doesn't lose the warm path.
...
- **`ON DELETE RESTRICT`** from exchanges/accounts/pairs/agents → users: you can't drop a user with live resources. Soft-disable via `users.is_active = false`. Hard-delete is an explicit, deliberate operation.
...
### Data model — hierarchy: `exchanges` → `accounts` → `pairs`

> **RESTRUCTURE (accounts-pairs-restructure):** the flat `projects` table was split into a 4-level hierarchy **Exchange → Account (Cuenta) → Pair (Par) → Agents** and the whole 0001–0014 migration history was **squashed into a fresh `0001_init`** (clean redesign, no data preservation). The old `projects` "trading unit" is now `pairs`; `project_id` is `pair_id` everywhere. `CHARTER.md` still describes the original flat `projects`/`agents` DDL as historical source material — when CHARTER.md and this hierarchy disagree, **this section + the live `0001_init` migration win**. The broker-credential block MOVED off the trading unit onto `accounts`.

**`exchanges`** — the trading venue / broker platform (top of the tree). Tenant-scoped (`user_id NOT NULL`, `ON DELETE RESTRICT`). An account belongs to exactly one exchange.

**`accounts`** (Cuenta) — a broker account under an exchange. **OWNS the broker-credential block** (`account_login`, `account_server`, `broker_name`, `account_credential_ref`, `account_currency`, `account_leverage`, `account_type`) — every pair under the account **inherits** these credentials; there is no per-pair credential override. `account_credential_ref` is **always** a pointer into an external secret store — never plaintext. Both FKs (`user_id`, `exchange_id`) are `ON DELETE RESTRICT`.

**`pairs`** (Par) — the trading unit (renamed from `projects`). One pair = one symbol + timeframe + strategy + capital + risk params, and one Docker/MT5/MCP container (see infra below). Belongs to one account (`account_id` FK `ON DELETE RESTRICT`) and keeps a denormalized `user_id NOT NULL` for direct tenant filtering. Things worth knowing without re-reading the migration:

- The schema uses Postgres features explicitly: `gen_random_uuid()`, `TIMESTAMP`, `TEXT[]`, `JSONB`. **Stack signal: the persistence DB is Postgres.** Don't pick a different DB without flagging it.
- `user_id` ties every pair to an owner — auth identity is the gate for visibility and control.
- `status` enum is `inactive | active | paused | stopped | error | maintenance` (canonical set + transition matrix in `apps/api/src/aether_api/services/pair_lifecycle.py`). `inactive` is the DDL default for new rows. Only `active` appears on the dashboard main view; the rest are managed under the Cuentas section. **There is no boolean `active` column** — "is this pair active?" is always `status = 'active'`. Don't add a duplicate boolean; it will drift.
...
- `mcp_url` + `mcp_port` are the per-pair MT5/MCP endpoint — see container topology below.
...
- **Per-agent params** are **six JSONB columns**: `orchestrator_params`, `investigator_params`, `marker_params`, `worker_params`, `tutor_params`, `auditor_params`. Schema is owned by each agent and validated at startup. If per-agent config history (e.g. across Sleep Phases) gets dense, promote to a `pair_agent_configs` side table — don't bloat the JSONB with embedded history. *History note: the original migration 0010 added `orchestrator_params`; migration 0012 added `marker_params` (market-signal split out of the Investigador) and `tutor_params` (Sleep Phase split out of the Orchestrator). Both are folded into the squashed `0001_init`.*
...
Up to **six** agent slot definitions per pair, each carrying executable Python logic. Full DDL in `CHARTER.md` under "Modelo de Datos: tabla `agents`" (the FK slots there name the old `projects` table — they now live on `pairs`). Key things to internalise:
...
- **Reused across pairs.** A single `agents.id` may be referenced from many `pairs` rows via `orchestrator_agent_id` / `investigator_agent_id` / `marker_agent_id` / `worker_agent_id` / `tutor_agent_id` / `auditor_agent_id`. Per-pair tuning happens in the JSONB `*_params` columns on `pairs`, not by duplicating agent rows.
...
- **`ON DELETE RESTRICT`** from `pairs` → `agents` (all six FK slots). To remove an agent, set `is_active = false`. Hard-deletion only when no pair still references it.
...
- **Security gate (NOT in DDL but firm requirement):** executing `agents.logica` is arbitrary code execution. The backend MUST sandbox it — isolated subprocess, no host filesystem, network restricted to that pair's MCP endpoint. Capture this when writing the Worker spec; not optional.

### Per-pair infrastructure (Docker + MT5)

- **1 pair = 1 Docker container = 1 MT5 instance = 1 MCP endpoint.** This 1:1:1:1 isolation is an invariant — no shared containers across pairs.
- Base image is `mt5-base:latest` (overridable via `pairs.docker_image`). A **default Dockerfile** is parameterized from each pair's row (symbol, resources) plus the broker/account credentials inherited from the pair's `accounts` row.
- The Cuentas section of the dashboard exposes a **button to generate the default Dockerfile** from a pair's config. Users should not need to write Docker by hand to bring up a standard pair.
- Container lifecycle (`container_id`, `container_name`, `status`) is system-managed and always reflected back into the `pairs` row — treat the DB row as source of truth, the Docker daemon as a slave to it.

### Operativa surface (realtime + history per pair) — load-bearing

The Operativa tab (`apps/web/src/app/(dashboard)/cuentas/[accountId]/pares/[pairId]/operativa/page.tsx`) is the single live operational view per pair. It is fed by an extended `orders` table, a write-only sandbox proxy, and an in-process realtime bus. Things a future Claude must not get wrong:

- **`orders` table carries the extended lifecycle shape** (originally migration `0013_operativa_orders_extend.py`, now folded into the squashed `0001_init`): 8 nullable lifecycle columns (`open_time`, `open_price`, `close_time`, `close_price`, `commission`, `swap`, `profit_gross`, `profit_net`) plus a `meta_data JSONB NOT NULL DEFAULT '{}'` column. The `status` CHECK was widened to include `closed` and `cancelled`. Existing pre-extension rows stay valid because every new column is nullable. Do NOT introduce a separate `trade_orders` table — `orders` is the canonical row.
...
- **REST endpoints carry the same tenancy + audit contract as WS.** `GET /api/pairs/{id}/operativa/account-summary` and `GET /api/pairs/{id}/operativa/orders` cross-tenant → 404 (no existence leak) AND `log_cross_tenant_attempt` line. `account-summary` NEVER returns 5xx on MCP failure — it returns 200 with `mcp_status='unavailable'` so the dashboard degrades cleanly. DB-side P&L (`pnl_day`, `pnl_week`, `pnl_month`) always computes regardless of MCP state.
- **Prometheus observability.** `apps/api/src/aether_api/services/operativa_metrics.py` exposes `aether_operativa_ws_subscribers{pair}` (Gauge, set by LiveBus on subscribe/unsubscribe) and `aether_operativa_mcp_status{pair}` (Gauge, 1=available / 0=unavailable, set on every MCP status transition). The label was renamed `project`→`pair` in the accounts-pairs restructure. Both land on the process-global registry that the existing `/metrics` endpoint exposes.

### Chat assistant (project-chat) — load-bearing

The Chat tab (`apps/web/src/app/(dashboard)/cuentas/[accountId]/pares/[pairId]/chat/page.tsx`) is the per-pair conversational surface where the operator interrogates the pair's state, history, and learning artefacts in natural language (Spanish). v1 is **read-only from the LLM's perspective** — the assistant cannot mutate pair state. Write/action surface (`propose_action` / `execute_approved_action`) is OUT OF SCOPE and deferred to the sibling change `project-chat-actions`.

- **v1 = persisted, audited, multi-tenant Claude assistant.** One conversation row per logical thread (`chat_conversations`, scoped by `pair_id`), append-only message history (`chat_messages`), pre-installed empty side table for future approvals (`chat_action_proposals`). Tables originally landed in migration `0014_chat.py`, now folded into the squashed `0001_init`. Conversations are NEVER hard-deleted by API — soft-delete via `archived_at IS NOT NULL`. The audit trail is permanent.
- **5 read-only tools, dispatcher-resolved**: `get_project_status`, `get_recent_trades`, `get_sleep_reports`, `get_qtable_summary`, `get_semantic_rules` (the LLM-facing tool names are deliberately unchanged for catalogue stability — they operate on the pair). Each consults an existing repository under a frozen `ChatDispatchContext` (`user_id`, `pair_id`, `conversation_id`). **Tenancy gate** — tool schemas MUST NOT declare `user_id` / `pair_id` / `project_id` / `conversation_id` (catalogue test enforces this); the dispatcher MUST strip any LLM-supplied tenancy key before invoking the callable (the strip set keeps the legacy `project_id` key too, to reject forged legacy tenancy keys). Tested in `tests/services/chat/test_tools.py`.
...
- **Feature flags**: `AETHER_CHAT_ENABLED` (default false) + presence of `ANTHROPIC_API_KEY` together gate the router mount AND the lifespan `chat_aborted_sweeper`. When off, the chat sub-routes under `/api/pairs/{id}/chat/*` 404 (router not mounted) and `/api/health` reports `features.chat_enabled = false`. The sweeper is owned by `app.state.chat_sweeper_task`; shutdown drains it via `contextlib.suppress(CancelledError, Exception)`.
- **Multi-tenancy invariants**: every chat repo query enforces tenancy transitively through `pairs.user_id` (the `chat_conversations.user_id` column exists only to drive the `(user_id, created_at DESC)` partial index, not as the authority). Cross-tenant access to any of the six endpoints → 404 (no existence leak), exercised in `tests/routers/test_chat.py`. Repos use `BaseRepository._for_user(stmt, user_id)`.
...
- **Main dashboard view shows ONLY `status = 'active'` pairs.** Other statuses (inactive/paused/stopped/error/maintenance) are managed under the Cuentas sidebar section (drill in: Cuenta → Pares), not on the main view.
- **Sidebar has exactly four entries, in this order**: `Cuentas`, `Agentes`, `Skills`, `Configuración`. (`Cuentas` was renamed from `Proyectos` in the accounts-pairs restructure; the route is `/cuentas`.) Don't add or rename without explicit user approval — this is product-fixed navigation.
...
- Maintain live pair state: positions, equity, drawdown, metrics, current phase.
