"""Sell node — appends a short-entry signal for the current bar."""
...
def generate(node: Node, connections: Connections) -> str:
    """Emit a sell signal dict (lots / sl / tp / pip + risk metadata)."""
    return trade_signal(node, "sell", connections)
