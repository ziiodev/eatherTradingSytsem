"""Registry mapping node type -> per-node generator callable.

This is the single place that wires node types to their modules. Adding a node
type = importing its module and adding one entry here; no central switch.
"""
...
from aether_api.services.codegen.nodes import (
    bearish_cross,
    bullish_cross,
    buy,
    condition,
...
# Node type (as emitted by the frontend) -> generator function.
...
def get_generator(node_type: str) -> GeneratorFn | None:
    """Return the generator for `node_type`, or None if unknown.
