"""Sell node — opens a short market order."""
...
def generate(node: Node, connections: Connections) -> str:
    """Emit an MQL5 sell market order using the standard Trade library.
    ...
    # passthrough of the raw take_profit value. Sell TP sits BELOW the Bid.
    tp_prelude = ""
    tp_arg = tp
    if float(tp_pips) > 0:
        ...
        tp_prelude = (
            ...
            f"   double bidTp_{node_id(node)} = "
            f"SymbolInfoDouble(_Symbol, SYMBOL_BID);\n"
            f"   double {tp_price} = bidTp_{node_id(node)} - {tp_dist};\n"
        )
        tp_arg = tp_price
    ...
    if ctx.has_risk_node and float(sl_pips) > 0:
        ...
        return (
            ...
            f"   double bid_{node_id(node)} = "
            f"SymbolInfoDouble(_Symbol, SYMBOL_BID);\n"
            f"   double {sl_price} = bid_{node_id(node)} + {sl_dist};\n"
            f"{tp_prelude}"
            f"   trade.Sell(CalcLots({ctx.risk_percent}, {sl_dist}), "
            f'_Symbol, 0.0, {sl_price}, {tp_arg}, "sell");'
        )

    return (
        f"{comment_header(node, NODE_TYPE)}\n"
        f"{tp_prelude}"
        f'   trade.Sell({lots}, _Symbol, 0.0, {sl}, {tp_arg}, "sell");'
    )
