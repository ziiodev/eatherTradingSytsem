"""Pair lifecycle state machine — canonical source of truth.

Renamed from ``project_lifecycle`` for the accounts-pairs hierarchy
(``Exchange → Account → Pair → Agents``). The old ``project_lifecycle``
module was removed in the clean redesign — import every symbol from here.

Co-owned with the ``project-docker-orchestration`` change, but THIS
module is the canonical owner of:

* the status set (``PAIR_STATUSES``)
...
#: Canonical, ordered tuple of every allowed pair status.
PAIR_STATUSES: Final[tuple[Status, ...]] = (
    "inactive",
    "active",
    "paused",
    "stopped",
...
#: Statuses from which a pair may be hard-deleted.
...
class InvalidTransition(ValueError):
    ...
    def __init__(self, from_status: str, to_status: str) -> None:
        ...
        super().__init__(
            f"invalid pair status transition: {from_status!r} -> {to_status!r}"
        )
...
def _coerce(status: str) -> Status:
    ...
    # Membership in PAIR_STATUSES (a tuple of literals) narrows ``status``
...
def can_transition(from_status: str, to_status: str) -> bool:
    ...
    """
    if from_status not in PAIR_STATUSES or to_status not in PAIR_STATUSES:
        return False
...
def is_deletable(status: str) -> bool:
    """Return True iff a pair in ``status`` may be hard-deleted."""
