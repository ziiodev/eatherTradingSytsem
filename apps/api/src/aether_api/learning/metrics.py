...
* :data:`q_table_promotions_total` (Counter) — incremented inside
  ...
  the savepoint releases successfully. Labels: ``project``,
...
* :data:`qtable_bytes` (Gauge) — set after every ``q_tables`` insert
  ...
  the project_id + byte count payload. Labels: ``project``.
...
#: only when the cache slot for ``(project_id)`` is older than this so
...
q_table_promotions_total: Counter = Counter(
    ...
    "Number of Q-Table version promotions, partitioned by project_id "
    "and the risk class assigned by the classifier.",
    labelnames=("project", "risk_class"),
)


episodic_rows: Gauge = Gauge(
    "aether_episodic_rows",
    "Number of rows in episodic_memory for a given project_id. Updated "
...
qtable_bytes: Gauge = Gauge(
    ...
    "given project_id. Crosses settings.learning_qtable_warn_bytes "
...
#: ``{project_id_str: monotonic_seconds_at_last_update}``. Process-global
...
def update_episodic_rows(
    ...
    *,
    ...
    """Set :data:`episodic_rows` for ``project_id`` to ``row_count``.

    Throttled by ``EPISODIC_GAUGE_TTL_SECONDS`` per project — callers
    ...
    """
    label = _stringify_project(project_id)
    now = time.monotonic()
    if not force:
    ...
    episodic_rows.labels(project=label).set(float(row_count))
...
def record_qtable_bytes(
    ...
    qtable_bytes.labels(project=label).set(float(size))
...
def increment_q_table_promotion(
    ...
    """Bump :data:`q_table_promotions_total` for ``(project, risk_class)``.
    ...
    """
    label_project = _stringify_project(project_id)
