"""Live MT5 endpoints attached to the pairs router.

Mounted from :mod:`aether_api.routers.pairs` as a sub-router so the
URL prefix stays ``/api/pairs``. Keeping the live surface here keeps
``pairs.py`` focused on CRUD + Docker orchestration and makes the
...
* ``GET  /{pair_id}/account``
* ``GET  /{pair_id}/positions``
* ``GET  /{pair_id}/history``
* ``GET  /{pair_id}/candles``
* ``GET  /{pair_id}/orders``
* ``POST /{pair_id}/orders``                          (CSRF, feature-flagged)
* ``GET  /{pair_id}/approvals``
* ``POST /{pair_id}/approvals/{approval_id}/approve`` (CSRF, admin or owner)
* ``POST /{pair_id}/approvals/{approval_id}/reject``  (CSRF, admin or owner)
* ``GET  /{pair_id}/operativa/account-summary``       (operativa surface)
* ``GET  /{pair_id}/operativa/orders``                (operativa surface)
"""
...
router = APIRouter(prefix="/api/pairs", tags=["pairs-live"])
...
async def _load_pair_or_404(
    pair_id: uuid.UUID, user: User, session: AsyncSession
) -> Any:
...
async def _load_operativa_pair_or_404(
    pair_id: uuid.UUID,
    user: User,
    ...
    *,
    ...
    """Variant of :func:`_load_pair_or_404` for the Operativa surface.

    Per ``sdd/pair-operativa/spec/multi-tenancy-delta`` (#2122) the
    operativa REST endpoints MUST emit a structured cross-tenant audit
    line when the caller is authenticated but the pair belongs to a
    different tenant. We can't distinguish "no such pair" from "wrong
    ...
    so we never leak pair existence.
    """
    repo = PairRepository(session)
    pair = await repo.get_for_user(user.id, pair_id)
    if pair is None:
...
@router.get("/{pair_id}/account")
async def get_project_account(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    pair = await _load_pair_or_404(pair_id, user, session)
    client = get_mcp_client(pair)
    try:
...
async def list_project_orders(
    ...
    await _load_pair_or_404(pair_id, user, session)
    repo = OrderRepository(session)
    rows = await repo.list_for_project(
        user_id=user.id, project_id=pair_id, limit=limit, offset=offset
    )
    total = await repo.count_for_project(user_id=user.id, project_id=pair_id)
    return {
        "items": [
...
def _decode_positions(payload: dict[str, Any]) -> list[PositionExposure]:
    ...
    for raw in payload.get("positions") or []:
        ...
        # Conservative: pair equity for percentage conversion is read
...
async def place_project_order(
    ...
    try:
        ...
        # Per spec: MCPUnreachable → flip pair to 'error' (best
...
async def list_project_approvals(
    ...
    await _load_pair_or_404(pair_id, user, session)
    rows = await list_pending_approvals(session, project_id=pair_id)
    return {
        "items": [
            {
                "id": str(r.id),
...
@router.post(
    "/{pair_id}/approvals/{approval_id}/approve",
    dependencies=[Depends(csrf_dependency)],
)
...
@router.post(
    "/{pair_id}/approvals/{approval_id}/reject",
    dependencies=[Depends(csrf_dependency)],
)
...
async def _sum_closed_pnl_since(
    ...
    *,
    ...
    """Sum ``profit_net`` over closed orders for the pair since ``since``.
...
@router.get(
    "/{pair_id}/operativa/account-summary",
...
async def get_operativa_account_summary(
    ...
    * Tenant-scoped: cross-tenant returns 404 (no existence leak).
    """
    pair = await _load_operativa_pair_or_404(
        pair_id, user, session, operation="operativa_account_summary"
    )
    ...
    pnl_day = await _sum_closed_pnl_since(
        session, pair_id=pair_id, user_id=user.id, since=day_since
    )
    pnl_week = await _sum_closed_pnl_since(
        session, pair_id=pair_id, user_id=user.id, since=week_since
    )
    pnl_month = await _sum_closed_pnl_since(
        session, pair_id=pair_id, user_id=user.id, since=month_since
    )
...
async def list_operativa_orders(
    ...
    * Defaults: ``from = now - 30d``, ``to = now`` — the canonical
    ...
    """
    await _load_operativa_pair_or_404(
        pair_id, user, session, operation="operativa_orders_list"
    )
