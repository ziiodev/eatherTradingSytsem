"""``/api/pairs`` — tenant-scoped CRUD + lifecycle for pairs.

End-to-end contract:

* ``GET /api/pairs``                 — list (filterable, paginated).
* ``GET /api/pairs/{id}``            — detail.
* ``POST /api/pairs``                — create (defaults to ``status='inactive'``).
* ``PATCH /api/pairs/{id}``          — partial update; status changes refused.
* ``DELETE /api/pairs/{id}``         — only when in a deletable state.
* ``POST /api/pairs/{id}/activate``  — explicit lifecycle transition.
* ``POST /api/pairs/{id}/pause``     — explicit lifecycle transition.
* ``POST /api/pairs/{id}/stop``      — explicit lifecycle transition.
* ``POST /api/pairs/{id}/mark-error``— explicit lifecycle transition.
* ``POST /api/pairs/{id}/maintenance``— explicit lifecycle transition.
...
from aether_api.models.pair import PAIR_STATUSES, TRADING_SESSIONS, Pair
...
class PairCreate(BaseModel):
    ...
    # Reparent: the owning account is REQUIRED. The broker-credential
    # block lives on the Account (Cuenta), never on the pair.
    account_id: uuid.UUID
...
class PairPatch(BaseModel):
    ...
    def _at_least_one_field(self) -> PairPatch:
...
# Mapping helpers
# ---------------------------------------------------------------------------
def _to_detail(row: Pair) -> PairDetail:
    return PairDetail.model_validate(row, from_attributes=True)


def _to_summary(row: Pair) -> PairSummary:
    return PairSummary.model_validate(row, from_attributes=True)


def _create_payload(body: PairCreate) -> dict[str, Any]:
...
async def list_pairs(
    ...
    """List the caller's pairs. Ignores any client-supplied user_id.
...
@router.get("/{pair_id}", response_model=PairDetail)
async def get_pair(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> PairDetail:
    """Fetch one pair by id. 404 if not found OR not owned (no leak)."""
    repo = PairRepository(session)
    row = await repo.get_for_user(user.id, pair_id)
    if row is None:
...
async def create_pair(
    ...
    """Create a pair owned by the current user.
    ...
    # Tenancy gate: the owning account MUST belong to the caller. A
    # foreign / non-existent account → 404 (no existence leak), never a
    # raw FK IntegrityError.
    account = await AccountRepository(session).get_for_user(
        user.id, body.account_id
    )
    if account is None:
        raise HTTPException(
...
# Explicit allowlist mirrors PairPatch — PairPatch's ``extra=forbid``
...
async def patch_pair(
    ...
    * Name collision with another pair of the same tenant → 409.
    ...
    if new_name is not None and await repo.name_taken_for_user(
        user.id, new_name, exclude_id=pair_id
    ):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="pair name already in use",
    ...
    updated = await repo.update_fields(user.id, pair_id, updates)
    if updated is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="pair not found")
    await session.commit()
...
async def delete_pair(
    ...
    """Hard delete. Allowed only when the pair is deletable (inactive | stopped).
    ...
    if not is_deletable(pair.status):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"pair status {pair.status!r} is not deletable",
        )
    if pair.container_id is not None:
        # Defence-in-depth: never delete a row that still references a live container.
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="pair still has a container_id; stop the container first",
        )

    await repo.delete(user.id, pair_id)
    await session.commit()
...
async def _transition(
    ...
    * 409 — current status disallows ``to_status`` per the state machine,
    ...
    try:
        assert_transition(pair.status, to_status)
    ...
    updated = await repo.update_status_if(
        user.id, pair_id, from_status=pair.status, to_status=to_status
    )
    if updated is None:
        # Someone moved the row in between — surface as 409 with a hint.
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="pair status changed by another request; retry after refresh",
        )
    await session.commit()
    await session.refresh(updated)
    return _to_detail(updated)
...
async def activate_pair(
    ...
    return await _transition(pair_id, user, session, to_status="active")
...
async def pause_pair(
    ...
    return await _transition(pair_id, user, session, to_status="paused")
...
async def stop_pair(
    ...
    return await _transition(pair_id, user, session, to_status="stopped")
...
async def mark_error_pair(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> PairDetail:
    return await _transition(pair_id, user, session, to_status="error")
...
async def maintenance_pair(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> PairDetail:
    return await _transition(pair_id, user, session, to_status="maintenance")


# ---------------------------------------------------------------------------
# Per-pair Docker orchestration endpoints
# ---------------------------------------------------------------------------
# These endpoints are ADDED by the ``pair-docker-orchestration`` change.
...
async def preview_dockerfile(
    ...
    """Render the default Dockerfile for ``pair_id`` without side effects.
    ...
    """
    repo = PairRepository(session)
    # render_default_dockerfile reads ``pair.account`` (lazy="raise") so
    # we MUST eager-load the account here or the render raises.
    pair = await repo.get_for_user_with_account(user.id, pair_id)
    if pair is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="pair not found")

    try:
        body = render_default_dockerfile(pair)
...
async def build_pair_image(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    """Render + build the pair image via the proxy.
    ...
    try:
        dockerfile_text = render_default_dockerfile(pair)
    except UnsafeValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"code": "unsafe_value", "field": exc.field, "value": exc.value},
    ...
    try:
        result = await build_image(
            session, user=user, pair_id=pair_id, dockerfile_text=dockerfile_text
        )
...
async def _container_op(
    ...
    *,
    ...
    try:
        result = await op(session, user=user, pair_id=pair_id)
    except ProjectNotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="pair not found"
        ) from None
...
@router.post(
    "/{pair_id}/container/create",
    dependencies=[Depends(csrf_dependency)],
)
async def create_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(pair_id, user, session, create_container, op_name="create")


@router.post(
    "/{pair_id}/container/start",
    dependencies=[Depends(csrf_dependency)],
)
async def start_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(pair_id, user, session, start_container, op_name="start")


@router.post(
    "/{pair_id}/container/pause",
    dependencies=[Depends(csrf_dependency)],
)
async def pause_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(pair_id, user, session, pause_container, op_name="pause")


@router.post(
    "/{pair_id}/container/stop",
    dependencies=[Depends(csrf_dependency)],
)
async def stop_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(pair_id, user, session, stop_container, op_name="stop")


@router.post(
    "/{pair_id}/container/recreate",
    dependencies=[Depends(csrf_dependency)],
)
async def recreate_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(
        pair_id, user, session, recreate_container, op_name="recreate"
    )
...
async def remove_pair_container(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict[str, Any]:
    return await _container_op(pair_id, user, session, remove_container, op_name="remove")


@router.get(
    "/{pair_id}/container/logs",
    response_class=PlainTextResponse,
)
async def get_pair_container_logs(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
    tail: Annotated[int, Query(ge=1, le=2000)] = 200,
) -> PlainTextResponse:
    """Return the tail of the pair container's stdout+stderr.
    ...
    try:
        body = await container_logs(
            session, user=user, pair_id=pair_id, tail=tail
        )
    except ProjectNotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="pair not found"
...
@router.get("/{pair_id}/container/events")
async def list_pair_container_events(
    pair_id: uuid.UUID,
    user: Annotated[User, Depends(current_user)],
    session: Annotated[AsyncSession, Depends(get_session)],
    ...
    """Paginated, newest-first feed of container_events for the pair."""
    repo = PairRepository(session)
    pair = await repo.get_for_user(user.id, pair_id)
    if pair is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="pair not found")
    ...
    rows = await events_repo.list_for_project(pair_id, limit=limit, offset=offset)
    total = await events_repo.count_for_project(pair_id)
    return {
        "items": [
            {
                "id": str(row.id),
...
# before reaching the per-pair MCP server. The feature flag
