"""End-to-end tests for the Operativa surface on ``pairs_live``.
...
These tests stub :func:`aether_api.routers.pairs_live.get_mcp_client`
...
async def test_account_summary_happy_path_mcp_up(app_client) -> None:
    ...
    with _patch_mcp_client():
        resp = await app_client.get(
            f"/api/pairs/{project_id}/operativa/account-summary"
        )
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
async def test_account_summary_cross_tenant_returns_404(app_client) -> None:
    ...
    resp = await app_client.get(
        f"/api/pairs/{project_a_id}/operativa/account-summary"
    )
...
async def test_orders_default_window_returns_all_recent(app_client) -> None:
    ...
    resp = await app_client.get(f"/api/pairs/{project_id}/operativa/orders")
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["total"] == 2
...
async def test_orders_filter_by_symbol(app_client) -> None:
    ...
    resp = await app_client.get(
        f"/api/pairs/{project_id}/operativa/orders",
...
async def test_orders_filter_by_magic_and_status(app_client) -> None:
    ...
    resp = await app_client.get(
        f"/api/pairs/{project_id}/operativa/orders", params={"magic": 42}
    )
    assert resp.status_code == 200
...
async def test_orders_cross_tenant_returns_404(app_client) -> None:
    ...
    resp = await app_client.get(
        f"/api/pairs/{project_a_id}/operativa/orders"
    )
...
async def test_account_summary_cross_tenant_emits_audit(
    ...
    from aether_api.routers import pairs_live as router_module
