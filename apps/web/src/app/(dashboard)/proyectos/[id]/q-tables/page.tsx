...
export default function QTablesPage({
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        const data = await fetchQTables(projectId, { limit: 100 });
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        const cur = await fetchQTable(projectId, selectedVersion);
        if (cancelled) return;
        setCurrent(cur);
        const prevVersion = selectedVersion - 1;
        if (prevVersion >= 1) {
          try {
            const prev = await fetchQTable(projectId, prevVersion);
