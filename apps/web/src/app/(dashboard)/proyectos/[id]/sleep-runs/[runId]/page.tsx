 ...
 *
 * Data source: GET /api/projects/{id}/sleep-runs/{runId}/report combined
 * with GET /api/projects/{id}/sleep/runs/{runId} for the run metadata
    ...
    const load = async (): Promise<void> => {
      try {
        const [runData, reportData] = await Promise.all([
          getSleepRun(projectId, runId),
          fetchSleepReport(projectId, runId),
        ]);
        if (cancelled) return;
        setRun(runData);
  ...
  return (
    ...
    >
      ...
      <span className="ml-auto text-xs text-[rgb(var(--foreground-muted))]">
        run_id: <code>{runId}</code> · proyecto: <code>{projectId}</code>
      </span>
    </header>
  );
}
    ...
    <Card data-testid="report-qtable-versions">
      ...
      <CardContent className="flex items-center gap-2">
        <Link
          href={
            before !== null
              ? `/proyectos/${projectId}/q-tables#v${before}`
              : `/proyectos/${projectId}/q-tables`
          }
        ...
        <Link
          href={
            after !== null
              ? `/proyectos/${projectId}/q-tables#v${after}`
              : `/proyectos/${projectId}/q-tables`
          }
          className="rounded-md border border-[rgb(var(--border))] px-3 py-1 text-sm hover:bg-[rgb(var(--background-elevated))]"
...
function BackLink({ projectId }: { projectId: string }): React.JSX.Element {
  return (
    <Link
      href={`/proyectos/${projectId}/sleep-runs`}
