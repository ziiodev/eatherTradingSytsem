...
export default function ChatPage({
  ...
  return <ChatPageInner pairId={pairId} />;
}
...
export function ChatPageInner({ pairId }: ChatPageInnerProps): React.JSX.Element {
  ...
  const refreshConversations = useCallback(async () => {
    setLoadingList(true);
    try {
      const data = await listConversations(pairId, {
  ...
  const handleCreate = useCallback(async () => {
    try {
      const conv = await createConversation(pairId, {});
  ...
  const handleArchive = useCallback(
    async (id: string) => {
      try {
        await patchConversation(pairId, id, { archived: true });
  ...
  const handleModelChange = useCallback(
    async (model: ChatModel) => {
      if (!activeId) return;
      try {
        const updated = await patchConversation(pairId, activeId, {
