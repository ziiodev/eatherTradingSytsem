    ...
    <section className="flex flex-col gap-4">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
        ...
        <TabsContent value="episodica">
          <EpisodicTab pairId={pairId} />
        </TabsContent>
        <TabsContent value="semantica">
          <SemanticTab pairId={pairId} />
        </TabsContent>
      </Tabs>
    </section>
  );
...
function EpisodicTab({
  ...
  const load = useCallback(async (): Promise<void> => {
    setLoading(true);
    try {
      const data = await fetchEpisodicMemory(pairId, {
...
function SemanticTab({
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        const data = await fetchSemanticMemory(pairId, { active: true });
        if (cancelled) return;
        setRules(data.items);
        setError(null);
      } catch (err) {
