...
/**
 * PairAgentsPanel — visible on the pair detail page. Renders the
 ...
 *
 * Charter: a pair has at most one agent of each type. The backend
 * doesn't enforce "no duplicate id across slots" — the dialog flags it
 * client-side via a warning.
 */

export interface PairAgentsPanelProps {
  pair: PairDetail;
...
export function PairAgentsPanel({
  pair,
  onPairUpdated,
}: PairAgentsPanelProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      const ids = [
        ...
        pair.worker_agent_id,
        pair.tutor_agent_id,
        pair.auditor_agent_id,
      ...
      if (pair.orchestrator_agent_id) types.add("orchestrator");
      if (pair.investigator_agent_id) types.add("investigator");
      if (pair.marker_agent_id) types.add("marker");
      if (pair.worker_agent_id) types.add("worker");
      if (pair.tutor_agent_id) types.add("tutor");
      if (pair.auditor_agent_id) types.add("auditor");
      try {
  ...
  return (
    ...
    >
      ...
      <ul
        ...
        data-testid="pair-agents-list"
      >
        {SLOTS.map((slot) => {
          const agentId = pair[slot.key];
          ...
          return (
            <li key={slot.key}>
              <button
                ...
                data-testid={`pair-agents-row-${slot.type}`}
      ...
      {/* `key` forces a remount on every (re-)open so the dialog's
          ...
          parent pair) also re-seeds the dialog state if the user
          re-opens it. */}
      <EditAgentBindingsDialog
        key={`${dialogOpen ? "open" : "closed"}:${pair.orchestrator_agent_id ?? ""}:${pair.investigator_agent_id ?? ""}:${pair.marker_agent_id ?? ""}:${pair.worker_agent_id ?? ""}:${pair.tutor_agent_id ?? ""}:${pair.auditor_agent_id ?? ""}`}
        pair={pair}
        open={dialogOpen}
        onOpenChange={setDialogOpen}
        onSaved={onPairUpdated}
      />
    </div>
  );
}
...
// via PATCH /api/pairs/{id} with the six *_agent_id fields. Empty
...
function EditAgentBindingsDialog({
  ...
  // dialog opens or the underlying pair slot ids change — which
  ...
  const dirty =
    orchestratorId !== (pair.orchestrator_agent_id ?? "") ||
    investigatorId !== (pair.investigator_agent_id ?? "") ||
    markerId !== (pair.marker_agent_id ?? "") ||
    workerId !== (pair.worker_agent_id ?? "") ||
    tutorId !== (pair.tutor_agent_id ?? "") ||
    auditorId !== (pair.auditor_agent_id ?? "");
  ...
  async function save(): Promise<void> {
    setSubmitting(true);
    try {
      const updated = await patchPair(pair.id, {
      ...
      <Dialog open={open} onOpenChange={(next) => (next ? onOpenChange(true) : requestClose())}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Vincula el par con una definición de Orquestador,
