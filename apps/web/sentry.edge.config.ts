/**
 * Edge-runtime Sentry bootstrap (Next.js middleware, edge route handlers).
 * Loaded by ``instrumentation.ts`` when ``NEXT_RUNTIME === "edge"``.
 *
 * The edge runtime is restricted (no Node APIs); the Sentry SDK exposes
 * a smaller surface here. Tracing / replay are off because OTel handles
 * tracing server-side and the edge has no DOM.
 */
