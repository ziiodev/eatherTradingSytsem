...
Toda la lógica de trading **del Worker** vive en el campo `logica` del agente correspondiente (ver tabla `agents`) y se ejecuta en Python dentro del backend; las órdenes resultantes se envían a MT5 mediante el servidor MCP local (`./mcp`). Este plano de ejecución de los agentes **no cambia**: `agents.logica` sigue forzado a runtime `'python'`.

**Excepción acotada y sancionada — Gestión de EAs (decisión del operador, 2026-06-19):** la antigua regla absoluta "nunca generas código MQL5 y el sistema nunca crea Expert Advisors" queda **relajada** por una decisión explícita y fechada del operador que autoriza la funcionalidad **`ea-management`** (fusiona el editor visual de EAs de newTCN). Alcance preciso de la excepción: el editor visual de EAs (sección **"Gestión EAs"** del sidebar) **puede generar código fuente MQL5 y Python de Expert Advisors como artefactos del usuario**, y el sistema **puede persistirlos, versionarlos y permitir que el usuario los descargue/exporte**. Invariantes que **se preservan** (siguen siendo ciertos, NO se relajan): el EA generado es un **carril de artefacto separado**, nunca lógica de ejecución de agente; el puente código-generado→Worker y **cualquier despliegue automático de un EA generado a una instancia MT5 en vivo quedan FUERA DE ALCANCE / pospuestos a un cambio futuro**.
...
Reglas duras:
- **Plano de ejecución de agentes Python-only**: toda la lógica de trading del Worker vive en `agents.logica`, se ejecuta en Python en el backend (runtime forzado a `'python'` por CHECK) y MT5 sólo recibe órdenes vía MCP. La antigua regla absoluta "No MQL5 / el sistema no crea EAs" queda relajada por las dos excepciones acotadas de abajo; el plano de ejecución de agentes en sí no cambia.
  - **Excepción acotada y sancionada — Gestión de EAs (decisión del operador, 2026-06-19)**: la funcionalidad `ea-management` (editor visual "Gestión EAs", fusionado desde newTCN) **puede generar código fuente MQL5 y Python de Expert Advisors como artefactos del usuario**, y el sistema **puede persistirlos, versionarlos y permitir descarga/exportación**. Es un **carril de artefacto separado**, nunca lógica de ejecución de agente: `agents.logica` sigue siendo Python-only. El puente código-generado→Worker y **el despliegue automático de un EA generado a MT5 en vivo quedan FUERA DE ALCANCE / pospuestos a un cambio futuro**.
...
Notas de uso:
- **El campo `logica` es la pieza central**: contiene el código Python que define el comportamiento del agente. Para el Worker es la lógica de trading (señales → órdenes vía MCP). Para el Investigador es la lógica de lectura/resumen de noticias (post-0012). Para el Marker es la generación de señal de mercado. Para el Tutor es la coordinación de la Fase de Sueño. Para el Auditor es la lógica de evaluación de métricas/anomalías + q-table + informes MT5. **Sin MQL5 jamás en este carril** — `runtime` está constrained a `'python'` por DB; el plano de ejecución de agentes nunca ejecuta MQL5. (La generación de EAs MQL5/Python por el editor "Gestión EAs" es un **carril de artefacto separado** sancionado por la decisión del operador 2026-06-19; no toca `agents.logica` ni se auto-despliega a MT5.)
